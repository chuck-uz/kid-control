...
- [ ] **Автообновление ВЫКЛ по умолчанию (`Update.AutoInstall=false`):** служба проверяет
      релизы и уведомляет в Telegram, но НЕ ставит сама (авто-установка на месте могла подвесить
      службу). Обновление — вручную через `update.bat` или кнопкой «⬇️ Обновить сейчас» в боте.
      Хочешь hands-off — поставь `Update:AutoInstall=true` в конкретной установке.
