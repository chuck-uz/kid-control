...
- [x] **T11. Бот в бекенде: устройства + меню.** Long-poll бота внутри бекенда; верхний
      ...
      *Готово:* `FleetBotBackgroundService` (long-poll Telegram в бекенде): верхний уровень —
      список устройств (онлайн-индикатор), выбор → per-device меню из 8 папок, все действия
      с device-id в callback. `FleetBotActions` — device-scoped операции через fleet-сервисы
      (статус, +время/сброс/выключение/перезагрузка через очередь команд; пауза/блок через
      desired; правило/интервалы/ночь/окно/целевая версия через политику; `/enroll`, отзыв).
      `DbAdminRegistry` — whitelist по таблице `admin` (защита от удаления последнего).
      Медиа помечены «Phase 2». 7 тестов (реестр админов + действия на InMemory: список/статус,
      enqueue, bump desired/policy, отзыв, код привязки, graceful для неизвестного устройства).
      DI-граф проверен загрузкой; live-Telegram — в T12 (нужен токен + реальный агент).

### Блок D. Выпуск
- [x] **T12. Деплой на VM.** docker-compose + Caddy (TLS) + Infisical на VM; смоук:
      ...
      *Развёрнуто на бою (VM Ташкент 157.22.133.185):* бекенд живёт на
      **https://kidcontrol.example.com** (валидный Let's Encrypt через общий Caddy-прокси, сеть
      `edge`). DNS `kidcontrol.example.com` создан в Cloudflare. Образ собран под linux/amd64 и
      перенесён `docker save|load` (VM не тянула SDK). `postgres`+`backend` (`restart:
      unless-stopped`), миграции применены, админ `65310731` засеян. Бот **@controlkid_bot**
      long-poll'ит внутри бекенда. Боевой смоук по HTTPS: `/health`+`/health/db`=200; enroll →
      policy-delta (40/20→50/10) → команда +30 с ack; падение БД → liveness 200 / readiness 503.
      Dockerfile исправлен (MinVer override), Caddy — опциональный compose-профиль,
      `deploy/DEPLOY.md` — runbook + чеклист. *Осталось пользователю:* открыть бот, `/enroll`,
      прописать `Fleet:Url`+код на реальном Windows-ПК (3 шага из DEPLOY.md).
...
### Блок E. Доработки после запуска (по мере эксплуатации)
- [x] **E1. Переименование устройства в боте.** Кнопка «✏️ Имя» в меню устройства →
      бот ждёт следующее сообщение как новое имя → `DeviceAdminService.RenameAsync`
      (аудит `device.rename`) + эндпоинт `POST /admin/devices/{id}/name`.
      *Готово:* заменяет автоматическое машинное имя (напр. `WIN-VDQ87A826JI`) на понятное.
- [x] **E2. Корректная версия агента в боте.** `AgentInfo` берёт `InformationalVersion`
      (MinVer-стамп, напр. `2.1.2`), а не `AssemblyVersion` (`2.0.0.0`).
