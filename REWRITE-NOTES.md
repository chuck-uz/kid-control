...
## Развёртывание одним файлом (Windows)

`deploy.bat` в корне делает всё сам: запрашивает права администратора → ставит
.NET 8 SDK (если нет) → проверяет машинный .NET 8 Desktop Runtime (ставит через
winget) → скачивает **свежий релиз** с GitHub → `build.ps1` (restore/build/test/
publish) → запускает инсталлятор.

1. Открой `deploy.bat`, вверху задай `KC_OWNER`/`KC_REPO` (репозиторий с v2).
2. Двойной клик. По умолчанию откроется мастер установки (токен/chat id вводишь там).
3. Для полностью тихой установки заполни `KC_BOT_TOKEN` и `KC_ADMIN_IDS` — установка пройдёт без окон.

Если релизов ещё нет — поставь `KC_SOURCE_MODE=branch` (возьмёт head ветки `KC_BRANCH`).

### Удаление

`uninstall.bat` в корне полностью сносит установленный экземпляр (без сборки и сети):
снимает защиту реестра → **graceful** `sc stop` (безопасно снимает флаг critical, без
BSOD) → удаляет службу → глушит UI → сбрасывает ACL и владение → удаляет
`C:\Program Files\KidControl` и `C:\ProgramData\KidControl`. Чтобы сохранить конфиг с
токеном и состояние — поставь `KC_KEEP_DATA=1` вверху файла.

### Обновление

`update.bat` в корне обновляет уже установленный экземпляр: скачивает свежую версию →
собирает → запускает инсталлятор в режиме `/update` (только бинарники). **Сохраняются**
`appsettings.json` (токен/админы) и `session_state.json` (текущий таймер ребёнка).
Если служба не установлена — падает с понятной ошибкой (тогда нужен `deploy.bat`).

### Репозиторий

v2 живёт в **`chuck-uz/kid-control`, ветка `v2`** (приватный; `main` = v1 не тронут).
Дефолты в `deploy.bat`/`update.bat` уже настроены на `KC_SOURCE_MODE=branch`,
`KC_BRANCH=v2`. Когда сделаешь GitHub Release с тегом `vX.Y.Z` — переключи на
`KC_SOURCE_MODE=release`.

## Первый запуск на Windows (вручную)
